...
void leave(int node_num, int node_successor, int node_predecessor, struct finger_table_entry* finger)
{
	...
	for( i = 1 ; i <= FINGER_SIZE ; i++ )
	{
		buf[0] = '\0';
		send_buf[0] = '\0';
		id = node_num - (int) pow( 2, (double)(i-1) ) + 1;
		if(id < 0)
			id += 256;
		
        //pred_of = findPredecessor(0, 0, node0_successor, id, finger);
		


		pred_of = findPredecessor(node_num, node_num, node_successor, id, finger);
		if(pred_of != node_num){	
			...
			remove_from = node_predecessor;

		if(remove_from != node_num){	
			sprintf( send_buf, "removenode %i %i %i", node_num, i, finger[1].node );
			fd =sendMessage( remove_from, send_buf );
			close(fd);
		}
	}
...
void remove_node( int node_num, int node_predecessor, int i, int replace, struct finger_table_entry* finger )
{
	...
	for(j = 1; j < FINGER_SIZE; j++){
	if( finger[j].node == node_num )
	{	
		char buf[64];
		finger[j].node = replace;
		if(flag == 1){	//only send once
